...
for ii=1:8:img_L
    for jj=1:8:img_W
        ...
        dc_diff = zig_m(1) - dc_fore;
        dc_fore = zig_m(1);
        %计算SSSS,并查码
        if dc_diff == 0
            dc_code_word = dc_code{1};
            
        else
            dc_SSSS = floor(log2(abs(dc_diff))) + 1;
            dc_index = dc_SSSS + 1;
            dc_code_word = dc_code{dc_index};
        end
        dc_bin = char(dec2bin(abs(dc_diff)));
        dc_been_code = strcat(dc_code_word, dc_bin);
        total_bitstream=strcat(total_bitstream,dc_been_code,',');
        ...
        for i=2:1:64%DC系数不处理
            ...
            elseif zig_m(i)==0
                ...
                if ZRL<16%ZRL是0到15
                    %由尾码得到SSSS
                    j=j+1;
                    %似乎是没用代码，能进到这个else里就说明zig_m(i) ~= 0;
                    if zig_m(i)==0
                        SSSS=0;
                    ...
                    codeunit(j,2)=0;
                    codeunit(j,3)=0;
                    %第二部分游程减去16，剩下两个正常%如果ZRL-16>16，则应该再执行一次上述代码，这里写个循环判断比较严谨
                    j=j+1;
                    if zig_m(i)==0
                        SSSS=0;
                    else
        ...
        for i=1:1:size(codeunit,1)%对codeunit每一行进行操作
            prefix=string(dec2hex(codeunit(i,1)))+'/'+string(dec2hex(codeunit(i,2)));%游程+ssss（十六进制）
